# Documentación técnica del Codificador Educativo de Instrucciones RISC-V
...
Los valores de `opcode`, `funct3` y `funct7` se extrajeron de *The RISC-V
Instruction Set Manual, Volume I: User-Level ISA*, versión 20191213, capítulo 2 (codificación
...
a su representación sin signo de N bits (`to_unsigned`, ver §3). Por ejemplo,
...
- **S**: `imm[11:5]` en los bits `31-25`, `imm[4:0]` en los bits `11-7`. El
  campo `rd` (usado por R/I en `11-7`) no existe en S, ya que S no escribe en
...
- **`encode_i`**: el inmediato de 12 bits va completo en `[31:20]`, sin
  partir. Una sola función sirve para `addi`/`andi`/`lw`/`lb`, ya que la
...
- **`encode()`**: dispatch por diccionario (`{"R": encode_r, ...}`) sobre
  `instr.fmt`, en vez de una cadena `if/elif`, ya que el formato ya lo
  determinó el parser.
...
- `explain_instruction(text, word)`: arma el texto de salida (formato,
  binario de 32 bits, tabla de campos con rango, binario, decimal y rol,
  leída con `extract_bits` sobre la palabra ya codificada) y, para S/B, una
  línea adicional con el inmediato reensamblado, tomado directamente de
...
directamente del texto de la instrucción, ya que para I no hace falta
...
soportadas (mínimo 36 casos, 12 instrucciones × 3 casos cada una: positivo,
negativo, límite, cubriendo además `x0`, inmediatos máximo/mínimo
representables y desplazamiento cero en saltos).
